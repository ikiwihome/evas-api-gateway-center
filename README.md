...
A single-file Web UI (React + TypeScript) for operating and troubleshooting the **EVAS Proxy API** via its **Management API** (config, credentials, logs, and usage).

[中文文档](README_CN.md)

**Main Project**: https://github.com/ikiwihome/evas-api-gateway-center

## What this is (and isn’t)

- This repository is the Web UI only. It talks to the EVAS Proxy API **Management API** (`/v0/management`) to read/update config, upload credentials, view logs, and inspect usage.
- It is **not** a proxy and does not forward traffic.

## Quick start

### Option A: Use the Web UI bundled in EVAS Proxy API (recommended)

1. Start your EVAS Proxy API service.
...
Open `http://localhost:5173`, then connect to your EVAS Proxy API backend instance.
...
- For EVAS Proxy API bundling, the release workflow renames it to `management.html`.
